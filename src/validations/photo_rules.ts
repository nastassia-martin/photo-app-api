...
export const createPhotoRules = [
    ...
    body('comment').isString().bail().isLength({ min: 3 }).withMessage("Your comment is too short!")
]
...
export const updatePhotoRules = [
    ...
    body('comment').optional().isString().bail().isLength({ min: 3 }).withMessage("Your comment is too short!")
]